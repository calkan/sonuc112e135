...
keşifsel (heuristic) geliştirilmiş, ayrıca bu problem 
yakınsama algoritmaları geliştirilmesine olanak sağlayacak şekilde tekrar tanımlanmıştır.
Kısaca genom birleştirme için dört farklı yöntem vardır: 1) açgözlü (greedy) yöntemler, 2) örtüşme-düzenleme-oydaşma çizgeleri, 3) de Bruijn çizgeleri, ve 4) dizi çizgeleri. Bu yaklaşımların geniş özetleri ve karşılaştırmaları çok sayıda özet makalede sunulmuştur~\cite{Alkan2011b,Chaisson2015,Miller2010,Nagarajan2013,Zhang2011}. Ayrıca bazı algoritmaların listesini bu raporda sunuyoruz (Tablo~\ref{tab:algs}).

\begin{table}[]
\caption{Bazı genom birleştirme algoritmaları.}
\label{tab:algs}
\begin{center}
\begin{tabular}{l|l|l|l}
\textbf{Algoritma tipi}                                                   & \textbf{Yaygın kullanımı}                                                           & \textbf{Algoritma ismi} & \textbf{Referans} \\
\hline
Açgözlü                                                                    & Az tekrarlı genomlar                                                                & TIGR                    & \cite{Sutton1995}                  \\
                                                                           &                                                                                     & phrap                   &   \cite{phrap}                \\
\hline
\begin{tabular}[c]{@{}l@{}}Örtüşme-düzenleme-oydaşma \\ (ÖDO)\end{tabular} & \begin{tabular}[c]{@{}l@{}}Az hatalı, orta uzunlukta\\ okumalar\end{tabular}        & Celera                  &  \cite{Venter2001}                 \\
                                                                           &                                                                                     & PCAP                    &  \cite{Huang2003}                 \\
                                                                           &                                                                                     & ARACHNE                 & \cite{Batzoglou2002}                  \\
                                                                           &                                                                                     & Phusion                 &  \cite{Mullikin2003}                 \\
\hline
de Bruijn çizgeleri                                                        & Kısa okumalar                                                                       & EULER-SR                & \cite{Chaisson2004,Chaisson2008,Chaisson2009}                  \\
                                                                           &                                                                                     & Velvet                  &   \cite{Zerbino2008}                \\
                                                                           &                                                                                     & ALLPATHS-LG             &  \cite{Gnerre2011}                 \\
                                                                           &                                                                                     & SOAPdenovo              &  \cite{Li2010b}                 \\
                                                                           &                                                                                     & Cortex                  &  \cite{Iqbal2012}                 \\
                                                                           &                                                                                     & SPAdes                  &  \cite{Bankevich2012}                \\
                                                                           &                                                                                     & ABySS                   &  \cite{Simpson2009}                 \\
                                                                           &                                                                                     & Meraculous              & \cite{Chapman2011}                  \\
                                                                           &                                                                                     & HipMer                  & \cite{Georganas2015}                  \\
\hline
Dizi çizgesi                                                               & \begin{tabular}[c]{@{}l@{}}ÖDO ve de Bruijn \\ yöntemlerinin birleşimi\end{tabular} & SGA                     &   \cite{Simpson2012}                \\
                                                                           &                                                                                     & Falcon                  &  $\ast$                 \\
\hline
Hibrit                                                                     & \begin{tabular}[c]{@{}l@{}}Farklı dizi türlerinin \\ kullanımı\end{tabular}         & Cerulean                &   \cite{Deshpande2013}                \\
                                                                           &                                                                                     & Ray                     &   \cite{Boisvert2010}                \\
                                                                           &                                                                                     & hybridSPAdes            &  \cite{Antipov2016}                \\
\hline
\end{tabular}
\end{center}
$^\ast$ \url{https://github.com/PacificBiosciences/falcon}
\end{table}


Hibrit veri kullanımı ile genom birleştirmesi yapan algoritmaları ise üç ana gruba ayırabiliriz. Birinci gruptakiler, yüksek miktarda dizileme hatalarını barındıran üçüncü nesil dizileme teknoloji (PacBio, ONT)
 verilerindeki hataları kısa veri üreten ancak dizileme doğruluğu yüksek Illumina verileri ile on düzeltmesini yaparak, düzeltilmiş uzun okumalardan genom birleştiren algoritmalardır. Bu algoritmaların başlıcası olarak PacBioToCA~\cite{Koren2012} sayılabilir. Ikinci gruptakiler genom birleştirmesi için kullanılacak verileri hiyerarşik bir sırada ekleyerek kullanır, bunların başlıca örnekleri Ray~\cite{Boisvert2010}  algoritmasıdır. 
Üçüncü gruptakiler ise bu verileri farklı veri yapılarında tutarak eşzamanlı olarak kullanan birleştirme algoritmalarıdır. Üçüncü grubun en önemli örnekleri olarak Cerulean~\cite{Deshpande2013} ve 
hybridSPAdes~\cite{Antipov2016} 
 algoritmaları sayılabilir. Ayrıca aynı ya da farklı veri türlerini yinelemeli olarak kullanarak genom birleştirmesini düzelten Quiver ve HGAP~\cite{Chin2013} ve Nanopolish~\cite{Loman2015} algoritmaları da yaygın olarak kullanılmaktadır. 
